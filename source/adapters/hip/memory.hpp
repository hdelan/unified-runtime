...
struct BufferMem {
  ...
  /// The offset from the parent buffer base ptr
  size_t SubBufferOffset = 0u;
...
public:
  ...
  native_type getPtrWithOffset(const ur_device_handle_t Device, size_t Offset) {
    assert(allocateMemObjOnDeviceIfNeeded(OuterMemStruct, Device) ==
           UR_RESULT_SUCCESS);
...
/// with a call to getPtr/getArray which will allocate a new image on device if
/// need be allocateMemObjOnDeviceIfNeeded which should be called either at:
///
...
struct ur_mem_handle_t_ {
  ...
  ur_mem_handle_t_(ur_mem Parent, size_t SubBufferOffset)
    ...
    SubBuffer.SubBufferOffset = SubBufferOffset;
